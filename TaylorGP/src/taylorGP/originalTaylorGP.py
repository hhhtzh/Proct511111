...
def CalTaylorFeatures(f_taylor,_x,X,Y,population,Generation,Pop,repeatNum,eq_write):
    ...
    if X.shape[1]>1:
        if metric.judge_additi_separability():
            ...
            low_mse1, f_add1,population1 = CalTaylorFeatures(metric.f_left_taylor, metric._x_left, metric.X_left, metric.Y_left,population,Generation,Pop//2,repeatNum,eq_write)
            low_mse1 = low_mse1[0]
            f_add1 = f_add1[0]
            print('===========================start right recursion============================')
            low_mse2, f_add2,population2 = CalTaylorFeatures(metric.f_right_taylor, metric._x_right, metric.X_right, metric.Y_right,population,Generation,Pop//2,repeatNum,eq_write)
            # if population2 != None:
            low_mse2 = low_mse2[0]
            f_add2 = f_add2[0]
            ...
            print('===========================start left recursion============================')
            low_mse1, f_multi1,population1 = CalTaylorFeatures(metric.f_left_taylor, metric._x_left, metric.X_left, metric.Y_left,population,Generation,population,Pop//2,repeatNum,eq_write)
            # if population1 != None:
            low_mse1 = low_mse1[0]
            f_multi1 = f_multi1[0]
            print('===========================start right recursion============================')
            low_mse2, f_multi2,population2 = CalTaylorFeatures(metric.f_right_taylor, metric._x_right, metric.X_right, metric.Y_right,population,Generation,Pop//2,repeatNum,eq_write)
            # if population2 != None:
            low_mse2 = low_mse2[0]
            f_multi2 = f_multi2[0]
    ...
    return Taylor_Based_SR(_x,X,metric.change_Y(Y),qualified_list,eq_write,population,Generation,Pop,repeatNum,metric.judge_Low_polynomial())
...
def Taylor_Based_SR(_x,X,Y,qualified_list,eq_write,population,Gen,Pop,repeatNum,low_polynomial):
    ...
    if low_polynomial == False:
        ...
        tops_fit,tops_str,population,Y_pred = est_gp.fit(X, Y, qualified_list,population_input=population) #种群演化返回的list of适应度 和公式
...
def OriginalTaylorGP(X_Y,Y_pred,population,repeatNum,Generation,Pop,rmseFlag = False,qualified_list=None):
    ...
    if end_fitness == None:
        end_fitness, programs,population,Y_pred = Taylor_Based_SR(_x, X, change_Y(Y,qualified_list), qualified_list,
                                        eq_write,population,Generation,Pop, repeatNum,qualified_list[2] < 1e-5)
