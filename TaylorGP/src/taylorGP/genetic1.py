    ...
    def _verbose_reporter(self, run_details=None):
        ...
        if run_details is None:
            ...
            print(line_format.format(run_details['generation'][-1],
                                     ...
                                     oob_fitness,
                                     remaining_time))
    def fit(self, X, y,qualified_list,population_input=None,sample_weight=None):
        ...
        population_input : 如果有输入，俺就接着上一轮的演化
        Returns
        -------
        self : object
            Returns self.
        ...
        print("self.hall_of_fame and self.population_size:",self.hall_of_fame,self.population_size,sep = " ")
        ...
        qualified_list_print = qualified_list
        ...
        population.sort(key=lambda x: x.raw_fitness_)
        ...
        #            x22, x23,
        #            x24, x25, x26, x27, x28, x29,x30,x31,x32,x33,x34,x35,x36,x37,x38,x39,x40,x41,x42,x43,x44,x45,x46,x47,x48,x49])
        
        _x = [symbols(f'x{i}') for i in range(500)]
        set_value('_x', _x)

        _x = get_value("_x")
        # 将topk的适应度和str公式按list返回
        k = 3
        tops_fit = []
        tops_str = []
        for i,person in enumerate(population):
            tops_fit.extend([person.raw_fitness_])
            tops_str.extend([print_program(person.get_expression(), qualified_list_print, X,_x)])
            if i == k: break
        # tops_fit.insert(0,'1')
        return tops_fit,tops_str,population,population[0].execute(X) #将topk的适应度和str公式按list返回,population是最新一代的父母们
        # return self
