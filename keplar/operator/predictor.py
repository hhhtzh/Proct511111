...
from keplar.translator.translator import trans_op, trans_op1
...
class MetricsOperonBingoPredictor(Evaluator):
    def __init__(self, data, func_fund_list, metric="rmse", optimizer_method="lm"):
        super().__init__()
        self.optimizer_method = optimizer_method
        self.metric = metric
    ...
    def do(self, population=None):
        ...
        for i in range(len(self.func_fund_list)):
            for a, j in enumerate(self.func_fund_list[i]):
                str_eq = str(j)
                str_eq=trans_op1(str_eq)
                print(str_eq)
                bingo_ind = AGraph(equation=str_eq)
                bingo_ind._update()
                print(x)
                print(str(bingo_ind))
                arr = np.array(bingo_ind.evaluate_equation_at(x)).reshape(-1, 1)
