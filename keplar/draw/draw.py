...
class Draw:
    def __init__(self, types, file_path, names, dataName):
...
class BoxImgDraw(Draw):
    def __init__(self, types, file_path, names, dataName):
        super().__init__(types, file_path, names, dataName)

    def draw(self):
        if self.types == "csv":
            pd_df = pd.read_csv(self.file_path)
        else:
        ...
        bplot1 = axes.boxplot(pd_df,  # 描点上色
                              # notch=True,  # 箱体中位数凹陷
                              vert=True,  # 设置箱体垂直
        ...
        plt.xticks(np.arange(len(self.names)), self.names, fontsize=7, rotation=30)

        figPath = os.path.join('IMG_COLOR', 'LOG')
        plt.savefig(os.path.join(figPath, self.dataName + '.pdf'), dpi=720, bbox_inches='tight')
        plt.close()


class ParetoDraw(Draw):

    def __init__(self, types, file_path, names, dataName):
        super().__init__(types, file_path, names, dataName)

    def draw(self):
        if self.types == "feature":
            ...
            for col in [c for c in df_sum.columns if c.endswith('test') or c.endswith('size')]:
                ...
                df_sum[col + '_norm'] = df_sum.groupby('dataset')[col].apply(
