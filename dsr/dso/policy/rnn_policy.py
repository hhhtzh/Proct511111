...
# class LinearWrapper(tf.contilb.rnn_cell.LayerRNNCell):
# class LinearWrapper(tf.contrib.rnn.LayerRNNCell):
class LinearWrapper(tf.keras.layers.Wrapper):
#   def __init__(self, layer, **kwargs):
#     super(LinearWrapper, self).__init__(layer, **kwargs)
#     self.layer = layer
...
class LinearWrapper(tf.keras.layers.Wrapper):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.compat.v1.variable_scope(type(self).__name__):
            outputs, state = self.cell(inputs, state, scope=scope)
            # logits = tf.keras.layers.Dense(outputs, units=self._output_size)
        ...
        with tf.name_scope("controller"):
            ...
            def make_cell(name, num_units, initializer):
                if name == 'lstm':
                    ...
                    # return tf.compat.v1.keras.layers.LSTMCell(num_units, initializer=initializer)
                if name == 'gru':
                    return tf.compat.v1.nn.rnn_cell.GRUCell(num_units, kernel_initializer=initializer, bias_initializer=initializer)
                    # return tf.compat.v1.keras.layers.GRUCell(num_units, kernel_initializer=initializer, bias_initializer=initializer)
            ...
            # cell = tf.keras.layers.rnn.MultiRNNCell(         
