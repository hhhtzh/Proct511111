...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    def _tournament():
        ...
        contenders = random_state.randint(0, len(parents), 1)
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            if gen == 0:
                ...
                parents = []
                # parents.append(self._programs[gen - 1][0])
                parents.append(best_program)
                # print
                # parents = self._programs[gen - 1]
                # print("parents 0",best_program.raw_fitness_)
                # print("parents 1",fitness[1])
                # print("parents 2",fitness[2])
                # print("parents 0",parents[0].fitness)
            ...
            if self._metric.greater_is_better:
                best_program1 = population[np.argmax(fitness)]
            else:
                best_program1 = population[np.argmin(fitness)]
            

            if gen > 0:
                if best_program1.raw_fitness_ < best_program.raw_fitness_:
                    best_program = best_program1
            else:
                best_program = best_program1

            self.best = best_program
    ...
    def generate(self,X):
        ...
        if self.n_features_in_ != n_features:
            raise ValueError('Number of features of the model must match the '
                             ...
                             'n_features is %s.'
                             % (self.n_features_in_, n_features))
        X_new = np.array([gp for gp in self._best_programs])
        return X_new

    def fit_generate(self,X,y,sample_weight=None):
        return self.fit(X, y, sample_weight).generate(X)
