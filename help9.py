...
data = Data("txt_pmlb", "datasets/feynman/val/feynman-ii.38.14.txt", ["x0", "x1","y"])
...
# print(npx)
fit_list = []
with open('/home/tzh/PycharmProjects/pythonProjectAR5/zjw_result/feynman-ii.38.14_equ.csv', 'rt') as csvfile:
...
while i < len(rows):
    print(rows[i][1])
    # eval = SingleBingoEvaluator("R2", x, y, 1, True, op_equ=rows[i][1])
    eval = SingleBingoEvaluator(data, equation=rows[i][1])
    fit = eval.do()
    fit_list.append(fit)
    i += 1
fit_pd = pd.DataFrame({'MTaylor(zjw)': fit_list})
fit_pd.to_csv(r"zjw_result/feynman-ii.38.14_fit.csv", sep=',', mode="a")
